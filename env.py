...
if sys.platform == 'darwin':
    ...
    sys.path.append('/data/yaoxin/codes/vln/ref/Matterport3DSimulator/build')
import MatterSim
import csv
import numpy as np
import math
...
class R2RPanoBatch():
    ...
    def _shortest_path_action(self, state, goalViewpointId):
        ...
        target_rel = self.graphs[state.scanId].node[nextViewpointId]['position'] - np.array([state.location.x,state.location.y,state.location.z])
    ...
    def _pano_navigable(self, state, goalViewpointId):
        ...
        navigable[state.location.viewpointId] = {
            'position': np.array([state.location.x,state.location.y,state.location.z]),
        ...
        for viewpoint_id, weight in navigable_graph.items():
            ...
            target_rel = node['position'] - np.array([state.location.x,state.location.y,state.location.z])
